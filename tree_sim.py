...
if __name__ == "__main__":
    ...
    exit(0)
    l = 128
    test = ChangingPTreeSim(1e-3,4,5,3e4)
    test.set_p_change(1e-2, 1e4)
    test.set_p_change(1e-3, 2e4)
    test.run_sim()
    print(f"memory: {test.timer_memory_update.value}\nrequest_make: {test.timer_request_make.value}\nrequest_solve: {test.timer_request_solve.value}")
    results = test.summary_dict()
    print(f"enqueued: {results['stats'][0]}\texpired+completed: {results['stats'][1] + results['stats'][2]}\tdata: {len(results['request_times'])}")
    result_time, result_cycles = normalize_by_time(results["request_times"], results["request_cycles"], 128)
    plt.plot(result_time, result_cycles)
    plt.xlabel("Time (cycles)")
    plt.ylabel("Request Completion Time (cycles)")
    #plt.savefig("tree_change_p.png")
    plt.show()
    result_time, result_rate = normalize_by_time(results["request_times"], results["request_success"], 128)
    plt.plot(result_time, result_rate)
    plt.xlabel("Time (cycles)")
    plt.ylabel("Request Rate")
    plt.show()
    with open("tree_change_p.data", "wb") as f:
        pickle.dump(results, f)
    exit(0)

    pool = Pool()
    #for num_layers in tqdm([2,3,4,5,6,7,8]):
    for num_layers in tqdm([2,3,4,5]):
        params = [(num_layers, 0.1**(1 + 2*i/l)) for i in range(l)]
        for summary_dict in tqdm(pool.imap_unordered(run_tree_sim_with_params, params), total=l, position=1, leave=False):
        #for summary_dict in tqdm(map(run_tree_sim_with_params, params), total=l, position=1, leave=False):
            continue

    try:
        from notify import notify
        notify("Finished all the treesims!")
    except:
        print("All done!")
